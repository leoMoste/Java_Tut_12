...
public class Program {
	...
	public static void main(String[] args) {
		...
		do {
			...
			//nextInt() liest nur das n�chste Zeichen
			//nicht aber die Return-Taste
			//Folge:n�chstes NextLine() hier z.B. bei Eingabe
			//wird mit der Return-Taste "verbraten"
			//so dass die Namenseingabe �bersprungen wird
		...
		default: System.out.println("Kein Men�punkt");
	...
	private static void kundenauflistenSortiert() {
		// TODO Auto-generated method stub
		System.out.println("Wonach m�chten Sie sortieren?\n 1:Nach Name \n 2:Nach Knr \n 3:Nach Gebdat \n Ihre Wahl:");
		int wahl= s.nextInt();
		s.nextLine();
		
		//Mit Streams deutlich k�rzer l�sbar, da dort die Ausgangsliste von Haus aus nicht ge�ndert wird!!!
	    ...
	    kunden.stream().sorted((k1,k2)->k1.getKname().compareTo(k2.getKname())).forEach(Kunde::print);
			...
			System.out.println("Falsche Eingabe. Urspr�ngliche Reihenfolge:");
		}
	...
	private static void bearbeiten() {
		...
		for(Kunde k :kunden)
		{
			if(k.getKnr()==nr)
			{
				//Trennzeichen f�r Token auf Zeilenumbruch setzen  !Windows!
				//s.useDelimiter("\r\n");
				//String t=s.next(); l�sst dann auch Leerstellen zu
				s.useDelimiter("\r\n");
	...
	private static void loeschen() {
		// TODO Auto-generated method stub
		kundenauflisten();
		System.out.println("Geben Sie die zu l�schende Nummer ein:");
		int nr=s.nextInt();
		int i=0;
		for(Kunde k :kunden)
		{
	...
	public static void kundenauflisten() {
		for(Kunde k:kunden){
			//Alternativ toString()-Methode �berladen
			System.out.println(k.getKnr()+" "+k.getKname()+" "+k.getKgebdat());
		}
